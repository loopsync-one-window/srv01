...
export class AuthController {
  ...
  async googleCallback(@Req() req: any, @Res() res: Response) {
    ...
    if (user.accountType === 'VISITOR') {
      ...
      return res.redirect(
        `https://loopsync.cloud/open-account?${redirectParams.toString()}`,
      );
    } else if (user.accountType === 'CUSTOMER') {
      // Redirect to home page with tokens for CUSTOMER accounts
      return res.redirect(
        `https://loopsync.cloud/home?${redirectParams.toString()}`,
      );
    } else {
      ...
      return res.redirect(
        `https://loopsync.cloud/open-account?${redirectParams.toString()}&login=true`,
      );
    }
  }
