...
export class DevelopersService {
    ...
    async uploadPanCard(developerId: string, file: Express.Multer.File) {
        if (!this.s3Client) {
            throw new InternalServerErrorException("AWS S3 is not configured on the server.");
        }
    ...
    async getPayoutAccount(developerId: string) {
        const dev: any = await this.prisma.developer.findUnique({ where: { id: developerId } });
        if (!dev) throw new NotFoundException('Developer not found');
        return {
            id: dev.id,
    ...
    async getPayoutSchedule(developerId: string) {
        ...
        const now = new Date();
        const nextMonth = new Date(now.getFullYear(), now.getMonth() + 1, dev.payoutDay || 1);
    ...
    async exportRevenueReport(developerId: string, format: string, period: string) {
    ...
    async getAnalyticsOverview(developerId: string, range: string, region: string) {
    ...
    async getAnalyticsTraffic(developerId: string, range: string, region: string) {
    ...
    async getAnalyticsDevices(developerId: string, range: string, region: string) {
    ...
    async getOverviewSnapshot(developerId: string, search: string = "") {
        const dev = await this.prisma.developer.findUnique({ where: { id: developerId } });
        if (!dev) throw new NotFoundException('Developer not found');
    ...
    async createApp(developerId: string, data: { name: string; status?: string; color?: string }) {
    ...
    async getApp(developerId: string, appId: string) {
        ...
        if (!verifyKey) {
            try {
                const raw: any[] = await this.prisma.$queryRawUnsafe(`SELECT "verifyKey" FROM "App" WHERE "id" = '${appId}'`);
        ...
        if (verifyKey && typeof verifyKey === 'string' && verifyKey.startsWith('mock-')) {
        ...
        if (!verifyKey) {
            ...
            try {
                ...
                if ((updated as any).verifyKey) {
                    ...
                    await this.prisma.$executeRawUnsafe(`UPDATE "App" SET "verifyKey" = '${verifyKey}' WHERE "id" = '${appId}'`);
                    (app as any).verifyKey = verifyKey;
                }
            } catch (e) {
    ...
    async generateUploadUrl(developerId: string, appId: string, type: string, mimeType?: string, fileName?: string, size?: number) {
        await this.getApp(developerId, appId);
